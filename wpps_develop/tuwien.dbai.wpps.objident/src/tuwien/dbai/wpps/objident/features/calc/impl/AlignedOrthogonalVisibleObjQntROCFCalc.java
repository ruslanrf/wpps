...
import tuwien.dbai.wpps.objident.features.descr.AlignedOrthogonalVisibleObjQntROCFDesc;
 ...
 * @created Oct 23, 2012 8:03:28 PM
 */
@Singleton
public class AlignedOrthogonalVisibleObjQntROCFCalc extends AFeatureCalculator {
	private final IIEBasisAPI api;
	
	@Inject
	public AlignedOrthogonalVisibleObjQntROCFCalc(
			AlignedOrthogonalVisibleObjQntROCFDesc featureDescription
			...
			@Override public Boolean apply(IInstanceAdp avar) {
				if (contextObjs.contains(avar) && avar.canAs(IQntBlock.class))
					return ( qltTarget.hasRelation(avar, EBlockQltRelation.NORTH_ORTHOGONAL_VISIBLE_BLOCK_OF)
							...
							&& qltTarget.hasRelation(avar, EBlockQltRelation.ALIGNED_WITH);
				else return false;
		} } );
