 ...
 */
public class Cont10DQueryBasicRel extends AWPUWrapper {
	private static final Logger log = Logger.getLogger(Cont10DQueryBasicRel.class);

private static final String filePath = "/home/ruslan/tmp/eval/Cont10DQueryBasicRel.csv";
	
	/**
	 * @param description
	 */
	public Cont10DQueryBasicRel(AWPUMethodDescription description) {
		super(description);
	}
...
S.startQuerying();
		final IResults rez = api.union(
				...
				, api.getObjectsForQltBGMRelation(areaQnt, EBlockQltRelation.NTPPi)
				, api.getObjectsForQltBGMRelation(areaQnt, EBlockQltRelation.TPPi)
				, api.getObjectsForQltBGMRelation(
						new IPredicate<IInstanceAdp>() {
							@Override public Boolean apply(IInstanceAdp avar) {
								return !avar.equals(areaQnt); }}
						, areaQnt, EBlockQltRelation.EQUAL)
