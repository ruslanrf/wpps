...
package tuwien.dbai.wpps.core.spatialindex.jsi.rtree;
...
import tuwien.dbai.wpps.core.spatialindex.jsi.Point;
import tuwien.dbai.wpps.core.spatialindex.jsi.PriorityQueue;
import tuwien.dbai.wpps.core.spatialindex.jsi.Rectangle;
...
public class RTree implements SpatialIndex {
  ...
  private final RectangleDynamicUtils rectUtils;
  
  /**
   * Constructor. Use init() method to initialize parameters of the RTree.
   */
  public RTree(final RectangleDynamicUtils rectUtils) {  
    ...
    // all entries have been checked.
      ...
      if (!n.isLeaf()) {
        ...
        for (int i = startIndex; i < n.entryCount; i++) {
	  	    if (rectUtils.contains(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i],
  ...
  private void createNearestNDistanceQueue(Point p, int count, float furthestDistance) {
    ...
    while (parents.size() > 0) {
      ...
      if (!n.isLeaf()) {
        ...
        boolean near = false;
        for (int i = startIndex; i < n.entryCount; i++) {
          if (rectUtils.distanceSq(n.entriesMinX[i], n.entriesMinY[i], 
        ...
        for (int i = 0; i < n.entryCount; i++) {
          float entryDistanceSq = rectUtils.distanceSq(n.entriesMinX[i], n.entriesMinY[i],
                                                   n.entriesMaxX[i], n.entriesMaxY[i],
                                                   p.x, p.y);
          int entryId = n.ids[i];
  ...
  public void contains(Rectangle r, TIntProcedure v) {
    ...
    while (parents.size() > 0) {
      ...
      if (!n.isLeaf()) {
        ...
        for (int i = startIndex; i < n.entryCount; i++) {
          if (rectUtils.intersects(r.minX, r.minY, r.maxX, r.maxY, 
        ...
        for (int i = 0; i < n.entryCount; i++) {
          if (rectUtils.contains(r.minX, r.minY, r.maxX, r.maxY, 
                                 n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i])) {
  ...
  private Node splitNode(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY, int newId) {
    ...
    // debug code
    if (log.isDebugEnabled()) {
      float newArea = rectUtils.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY) + 
    		  rectUtils.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY);
  ...
  private int pickNext(Node n, Node newNode) {
    ...
    for (int i = 0; i < maxNodeEntries; i++) {
      if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
        ...
        float nIncrease = rectUtils.enlargement(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY, 
                                                n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]);
        float newNodeIncrease = rectUtils.enlargement(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY,
                                                      n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]);
        ...
        if (difference > maxDifference) {
          ...
          if (nIncrease < newNodeIncrease) {
            ...
            nextGroup = 1;
          } else if (rectUtils.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY) < rectUtils.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY)) {
            nextGroup = 0;
          } else if (rectUtils.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY) < rectUtils.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY)) {
            nextGroup = 1;
  ...
  private float nearest(Point p, Node n, float furthestDistanceSq) {
    for (int i = 0; i < n.entryCount; i++) {
      float tempDistanceSq = rectUtils.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i], p.x, p.y);
  ...
  private boolean intersects(Rectangle r, TIntProcedure v, Node n) {
    for (int i = 0; i < n.entryCount; i++) {
      if (rectUtils.intersects(r.minX, r.minY, r.maxX, r.maxY, n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i])) {
        if (n.isLeaf()) {
          if (!v.execute(n.ids[i])) {
            return false;
          }
  ...
  private Node chooseNode(float minX, float minY, float maxX, float maxY, int level) {
    ...
    while (true) {
      ...
      float leastEnlargement = rectUtils.enlargement(n.entriesMinX[0], n.entriesMinY[0], n.entriesMaxX[0], n.entriesMaxY[0],
                                                     minX, minY, maxX, maxY);
      ...
      for (int i = 1; i < n.entryCount; i++) {
        ...
        float tempEnlargement = rectUtils.enlargement(tempMinX, tempMinY, tempMaxX, tempMaxY, 
                                                      minX, minY, maxX, maxY);
        if ((tempEnlargement < leastEnlargement) ||
            ((tempEnlargement == leastEnlargement) && 
             (rectUtils.area(tempMinX, tempMinY, tempMaxX, tempMaxY) < 
            		 rectUtils.area(n.entriesMinX[index], n.entriesMinY[index], n.entriesMaxX[index], n.entriesMaxY[index])))) {
          index = i;
