 ...
 */
public class ContDQueryCompRel extends AWPUWrapper {
	private static final Logger log = Logger.getLogger(ContDQueryCompRel.class);

private static final String filePath = "/home/ruslan/tmp/eval/Cont14DQueryCompRelSIPRules.csv"; //TODO modify!
	
	/**
	 * @param description
	 */
	public ContDQueryCompRel(AWPUMethodDescription description) {
		super(description);
	}
...
S.startQuerying();
final IResults  rez2 = api.getSubjectsForQltBGMRelation(
		new IPredicate<IInstanceAdp>() {
			@Override public Boolean apply(IInstanceAdp avar) {
				return !avar.equals(areaQnt); }}
		, areaQnt, EBlockQltRelation.P);
S.stopQuerying();
System.out.println("2: "+S.getQueryingTS()+" cnt: "+rez.size());
