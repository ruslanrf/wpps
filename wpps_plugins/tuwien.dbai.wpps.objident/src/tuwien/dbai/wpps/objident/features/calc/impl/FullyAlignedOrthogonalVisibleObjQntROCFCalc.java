...
import tuwien.dbai.wpps.objident.features.descr.FullyAlignedOrthogonalVisibleObjQntROCFDesc;
 ...
 * @created Oct 23, 2012 8:14:32 PM
 */
@Singleton
public class FullyAlignedOrthogonalVisibleObjQntROCFCalc extends AFeatureCalculator {
	private final IIEBasisAPI api;
	
	@Inject
	public FullyAlignedOrthogonalVisibleObjQntROCFCalc(
			FullyAlignedOrthogonalVisibleObjQntROCFDesc featureDescription
			...
			@Override public Boolean apply(IInstanceAdp avar) {
				if (contextObjs.contains(avar) && avar.canAs(IQntBlock.class))
					return ( ( qltTarget.hasRelation(avar, EBlockQltRelation.NORTH_ORTHOGONAL_VISIBLE_BLOCK_OF)
							  ...
							  && qltTarget.hasRelation(avar, EBlockQltRelation.LEFT_ALIGNED_WITH)
							  && qltTarget.hasRelation(avar, EBlockQltRelation.RIGHT_ALIGNED_WITH)
							)
							|| ( (qltTarget.hasRelation(avar, EBlockQltRelation.EAST_ORTHOGONAL_VISIBLE_BLOCK_OF)
								...
								&& qltTarget.hasRelation(avar, EBlockQltRelation.TOP_ALIGNED_WITH)
								&& qltTarget.hasRelation(avar, EBlockQltRelation.BOTTOM_ALIGNED_WITH)
									);
				else return false;
		} } );
